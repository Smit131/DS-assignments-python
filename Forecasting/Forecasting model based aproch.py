import pandas as pd
Amtrak = pd.read_csv("G:\Data Science\Assn 18 - Forecasting\Python code\Amtrak.csv")
month =['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'] 
import numpy as np

# in the belo code ...the i converted month-year format in seperate month column without using datetime library which would have been much easier rather than waht i did..
p = Amtrak["Month"][0]
p[0:3]
Amtrak['months']= 0
...
predict_data = pd.read_csv("G:\Data Science\Assn 18 - Forecasting\Python code\Support\Predict_new.csv")
model_full = smf.ols('Ridership~t+t_squared+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov',data=Amtrak1).fit()

pred_full = pd.Series(model_full.predict(Amtrak1))
...
############################ Autocorrelation check in the errors ######################

e_series = (Amtrak1['Ridership'])-np.array(pred_full)

from matplotlib import pyplot
from statsmodels.graphics.tsaplots import plot_acf
plot_acf(e_series,lags=20)
pyplot.show()
